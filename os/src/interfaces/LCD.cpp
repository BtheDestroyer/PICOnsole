...
__attribute__((section(".piconsole.os.lcd_buffer"))) LCD_MODEL::buffer_type piconsole_lcd_buffer;

spi_inst_t *SPILCD::get_spi() { return LCD_SPI; }

bool SPILCD::init(bool final_step /* = true */)
{
    if (is_initialized())
    {
        return false;
...
void __debug_noinline SPILCD::write_data(std::initializer_list<std::uint8_t> bytes)
{
    write_data(std::span{bytes.begin(), bytes.end()});
}
...
void __debug_noinline SPILCD::write_command(std::initializer_list<std::uint8_t> bytes)
{
    write_command(std::span{bytes.begin(), bytes.end()});
}
...
bool PicoLCD_1_8::init(bool final_step /* = true */)
{
    if (!SPILCD::init(false))
    {
        return false;
    }
    //MX, MY, RGB mode (???)
