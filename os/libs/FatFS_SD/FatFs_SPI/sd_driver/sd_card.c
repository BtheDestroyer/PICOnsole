...
int sd_deinit(sd_card_t *pSD) {
    TRACE_PRINTF("> %s\r\n", __FUNCTION__);

    if (!mutex_is_initialized(&pSD->mutex)) mutex_init(&pSD->mutex);
    sd_lock(pSD);
    // Make sure we're already initialized before proceeding
    if (pSD->m_Status & STA_NOINIT) {
        sd_unlock(pSD);
        return pSD->m_Status;
    }
...
auto_init_mutex(sd_driver_mutex);

bool sd_init_driver() {
    mutex_enter_blocking(&sd_driver_mutex);
    if (!initialized) {
...
bool sd_deinit_driver() {
    mutex_enter_blocking(&sd_driver_mutex);
    if (initialized) {
        for (size_t i = 0; i < sd_get_num(); ++i) {
            sd_card_t *pSD = sd_get_by_num(i);
            if (pSD->use_card_detect) {
                gpio_deinit(pSD->card_detect_gpio);
            }
            gpio_deinit(pSD->ss_gpio);
            sd_deinit(pSD);
        }
        for (size_t i = 0; i < spi_get_num(); ++i) {
            spi_t *pSPI = spi_get_by_num(i);
            if (!my_spi_deinit(pSPI)) {
                mutex_exit(&sd_driver_mutex);
                return false;
            }
        }
